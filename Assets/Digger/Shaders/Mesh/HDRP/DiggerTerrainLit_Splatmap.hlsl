...
void TerrainSplatBlend(FragInputs input, inout TerrainLitSurfaceData surfaceData, PositionInputs posInput)
{
    float2 controlUV = input.texCoord0.xy;
    float2 splatBaseUV = controlUV;
    ...
    #define SampleNormalTriplanar(i) normal[i] = float3(0, 0, 0)
#endif
...
#define DefaultMask(i) float4(_Metallic##i, _MaskMapRemapOffset##i.y + _MaskMapRemapScale##i.y, _MaskMapRemapOffset##i.z + 0.5 * _MaskMapRemapScale##i.z, albedo[i].a * _Smoothness##i)
    ...
    #define SampleMasksTriplanar(i, blendMask) masks[i] = DefaultMask(i)
#endif

#define SampleResults(i, mask)                                                                                  \
    UNITY_BRANCH if (mask > 0)                                                                                  \
    {                                                                                                           \
        float2 tile = _Splat##i##_ST.xy;                                                      \
        ...
        float2 splatdxuv = dxuv;                                                             \
        float2 splatdyuv = dyuv;                                                             \
...
#define SampleResultsTriplanar(i, mask)                                                                         \
    ...
    float4 blendMasks0 = input.color;
    #ifdef _TERRAIN_8_LAYERS
        float4 blendMasks1 = float4(0, 0, 0, 0);
        blendMasks1.xy = input.texCoord2.xy;
        blendMasks1.zw = input.texCoord3.xy;
    #else
        float4 blendMasks1 = float4(0, 0, 0, 0);
    #endif
...
{
    TerrainSplatBlend(input, surfaceData, posInput);
}
